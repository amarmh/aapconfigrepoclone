...
name: submit-pr
description: Prepare and submit a pull request for the infra.aap_configuration Ansible Collection. Syncs with upstream, creates a feature branch, runs pre-commit and linting checks (ansible-lint, yamllint), updates documentation and changelogs as needed, commits with conventional commits, then creates the PR via gh against amarmh/aapconfigrepoclone. Use when the user asks to submit, create, or open a pull request, or says "submit PR", "open PR", "create PR".

Submit PR

## Repository configuration

This skill is scoped to **aapconfigrepoclone**:

| Setting | Value |
|---------|-------|
| Upstream repo | `amarmh/aapconfigrepoclone` |
| Base branch | `devel` |
| Fork (`origin`) | Your GitHub fork of `aapconfigrepoclone` |
| Upstream remote | `upstream` → `https://github.com/amarmh/aapconfigrepoclone.git` |

Ensure remotes are configured:

```bash
git remote -v
# origin   git@github.com:YOUR_GITHUB_USER/aapconfigrepoclone.git
# upstream https://github.com/amarmh/aapconfigrepoclone.git
```

If `upstream` is missing:

```bash
git remote add upstream https://github.com/amarmh/aapconfigrepoclone.git
```
...
gh pr create \
  --repo amarmh/aapconfigrepoclone \
...
Replace `YOUR_BRANCH_NAME` with the actual branch name. Do **not** run `gh pr create` without `--repo amarmh/aapconfigrepoclone` — that creates a PR on the fork instead of upstream.

Return the upstream PR URL to the user (e.g. `https://github.com/amarmh/aapconfigrepoclone/pull/NNNN`).

Maintaining the PR

When pushing additional commits to an existing PR, always update the PR body to reflect the new changes. Use `--repo amarmh/aapconfigrepoclone` for all `gh pr` commands:

```bash
gh pr edit PR_NUMBER --repo amarmh/aapconfigrepoclone --body "$(cat <<'EOF'
...updated body...
EOF
)"
```
